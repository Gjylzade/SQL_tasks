...
-- If INNER JOIN is changed to LEFT JOIN:
-- All orders would still appear, including those without order_items.
-- This would cause NULL values in total_value for orders that have no items.
-- 
SELECT 
    o.order_id,
...
-- Customers who have never placed an order would be removed from the result set.
-- Only customers with at least one order would be shown.
-- 
SELECT 
    c.customer_id,
    c.full_name,
...
-- Orders without a matching record in sales_rep_assignments would still appear,
-- showing NULL values for assigned_rep, which helps identify missing or incomplete assignment data.
-- 
SELECT 
    o.order_id,
    o.customer_id,
...
WHERE a.end_date IS NULL
  AND o.sales_rep_id IS DISTINCT FROM a.sales_rep_id;


-- 
-- J4: FULL order line report (5 tables)
-- If INNER JOIN is changed to LEFT JOIN:
-- Orders, customers, products, or sales reps without matching relationships would still appear.
-- This would help identify missing or incomplete relationships in the dataset, but may introduce NULL values in line-level calculations.
-- 
SELECT 
    o.order_id,
